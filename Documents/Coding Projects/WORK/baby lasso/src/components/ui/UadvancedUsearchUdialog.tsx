...
export interface AdvancedSearchState {
  ...
  imageSize?: NumberRangeValue; // e.g., width/height long edge
  matchMode?: 'any' | 'all';
}
...
export const AdvancedSearchDialog = ({ open, onOpenChange, state, onChange, onApply, onClear, className }: AdvancedSearchDialogProps) => {
  ...
  const setImageSize = (v: NumberRangeValue) => onChange({ ...state, imageSize: v });
  const setMatchMode = (m: 'any' | 'all') => onChange({ ...state, matchMode: m });
  ...
  return (
    ...
    >
      <div className={cn('grid gap-6 sm:grid-cols-2')}>
        ...
        <div className="sm:col-span-2">
          <div className="text-[11px] text-muted-foreground mb-2">Match</div>
          <SegmentedControl
            segments={[{label:'Match Any', value:'any'},{label:'Match All', value:'all'}] as any}
            value={(state.matchMode || 'any') as any}
            onChange={(v:any) => setMatchMode(v)}
          />
        </div>
      </div>
    </Modal>
