...
export class OpenApiExporter {
  ...
  private static createBaseSpec(apiVersion: string, serverUrl: string, dictionary: DataDictionary): any {
    return {
      openapi: '3.0.3',
      info: {
  ...
  private static parseExampleValue(example: string | number | boolean, type: string): any {
    ...
    switch (type) {
      ...
      case 'boolean':
        return typeof example === 'boolean' ? example : exampleStr.toLowerCase() === 'true'
      case 'array':
        try {
