...
export class JsonSchemaExporter {
  ...
  private static createEventExamples(event: DataDictionaryEvent) {
    ...
    if (event.properties.length > 0) {
      ...
      for (const property of event.properties) {
        if (property.example) {
          baseExample.properties[property.name] = this.parseExampleValue(String(property.example), property.type)
        }
      }
    }
    ...
    if (event.properties.some(p => p.required)) {
      ...
      for (const property of event.properties.filter(p => p.required)) {
        if (property.example) {
          minimalExample.properties[property.name] = this.parseExampleValue(String(property.example), property.type)
        }
      }
    }
