...
describe('OpenAIClient', () => {
  ...
  describe('createChatCompletion', () => {
    ...
    it('should handle timeout with retries', async () => {
      ...
      // Should have attempted 3 times (initial + 2 retries for timeout)
      expect(mockFetch).toHaveBeenCalledTimes(3)
      expect(callCount).toBe(3)
      
      // Should have retry progress updates
      const retryUpdates = progressUpdates.filter(u => u.state === 'retrying')
      expect(retryUpdates).toHaveLength(2) // 2 retries
      
      // Should have a final failed update
      const failedUpdate = progressUpdates.find(u => u.state === 'failed')
      expect(failedUpdate).toBeDefined()
      // The final message will be about max retries, but timeout events occurred
      expect(failedUpdate?.message).toContain('Request failed')
    })
  ...
  describe('validateOpenAIConfig', () => {
    ...
    it('should validate temperature range', () => {
      mockEnv.VITE_OPENAI_API_KEY = 'test-key'
